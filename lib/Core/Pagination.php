...
class Pagination {
    ...
    public function __construct($numbers, $pagesize=20, $show_count=1, $class_page='pagination justify-content-center'){
        global $smarty;
    ...
    function get_sql_limit($sql, $top_count = 0){
        if ($this->page == 1) {
            $sql .= ' LIMIT 0, '.($this->pagesize - $top_count);
        }
        else {
            $sql .= ' LIMIT '.(($this->page - 1) * $this->pagesize - $top_count).','.$this->pagesize;
        }
        return $sql;
    }
