	...
	<div class="row">
		<div class="col-md-6">
			...
			<div class="form-group">
				<input type="text" name="url" class="form-control" onchange="LoadPrefix(this.value);" placeholder="Nhập url để quét sản phẩm" value="{$page.website|default:''}">
			</div>
			<div class="form-group">
				<textarea name="prefix" class="form-control" rows="6" placeholder="Nhập các thông số để lấy nội dung. Ví dụ: name:.product_name&&code:.product_code">{$page.prefix_auto|default:''}</textarea>
			</div>
			<div class="form-group">
				<button type="button" class="btn btn-primary btn-lg" onclick="Save();">
					<i class="fa fa-save"></i> Save
				</button>
				<a href="?mod=scandb&site=autorun&id={$page.id|default:''}" class="btn btn-primary btn-lg">
					<i class="fa fa-play"></i> Scan New
				</a>
				<a href="?mod=scandb&site=autoupdate&id={$page.id|default:''}" class="btn btn-primary btn-lg">
					<i class="fa fa-fresh"></i> Update
				</a>
				<!-- <button type="button" id="StartAI" class="btn btn-primary btn-lg" onclick="StartAutorun();">
					<i class="fa fa-play"></i> Start Autorun
				</button> -->
				<button type="button" class="btn btn-primary btn-lg" onclick="GetContent();">
					<i class="fa fa-pause"></i> Get contents
				</button>
			</div>
		</div>
		<div class="col-md-6">
...
function GetContent(){
	...
	loading();
	$("#ShowPageContent .modal-body").load('?mod=scandb&site=load_pagecontent', data, function(e){
...
function Save(){
	...
	loading();
	$.post('?mod=pages&site=autorun', data).done(function(e){
