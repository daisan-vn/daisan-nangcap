		...
		<div class="col-md-4 text-right">
	        <div class="btn-group">
	            ...
	            <ul class="dropdown-menu dropdown-menu-right min127">
	                <li><a href="javascript:void(0);" onclick="LoadMedia('SetImgRls', {$out.category.id});">{$out.category.name}</a></li>
	            </ul>
	        </div>
            ...
            {foreach from=$result item=data}
                <tr id="field{$data.id}">
                    ...
                    <td class="text-right">
                        ...
                        <button type="button" class="btn btn-default btn-xs" title="Xóa" onclick="LoadDeleteItem('slider', {$data.id}, 'ajax_delete');"><i class="fa fa-trash fa-fw"></i></button>
                    </td>
                </tr>
            {/foreach}
            {else}
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
				<form>
					...
					<div class="form-group">
						<label>Chọn trang hiển thị</label> 
						<select class="form-control" name="position"></select>
					</div>
					<div class="form-group">
...
function filter(){
	...
	window.location.href = "?mod=posts&site=images" + "&cat=" + cat + "&position=" + position+ "&display=" + display;
}
