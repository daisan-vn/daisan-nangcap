    ...
    <div class="row">
        ...
        <div class="col-md-4 text-right">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu dropdown-menu-right min127">
                    <li><a href="javascript:void(0);"
                            onclick="LoadMedia('SetImgRls', {$out.category.id});">{$out.category.name}</a></li>
                </ul>
            </div>
...
<div class="row">
    <div class="col-md-9">
        <div class="form-group form-inline">
            <select class="left form-control" id="type">{$out.filter_type}</select>
            </select>
        ...
        <tbody id="body_img">
            ...
            <tr id="field{$data.id}">
                <td class="text-center">
                    <img class="imgrls" id="img{$data.id}" src="{$data.avatar}"
                        onclick="ViewImage('{$data.media_id}', {$data.id});">
                </td>
                <td>
                    {if $data.title}<p>{$data.title}</p>{/if}
                    {if $data.alias}<p class="text-small"><span><i class="fa fa-link fa-fw"></i> {$data.alias}</span>
                    </p>{/if}
                </td>
                <td>{$data.taxonomy}</td>
                <td>{$data.position}</td>
                <td><input class="form-control sort_input" type="number" name="sort" value="{$data.sort}" min="0"
                        max="9999" oninput="sortItem('posts', {$data.id}, this.value);" /></td>
                <td class="text-center" id="stt{$data.id}">{$data.status}</td>
                <td class="text-right">
                    <button type="button" class="btn btn-default btn-xs" onclick="LoadFormImage({$data.id});"><i
                            class="fa fa-pencil fa-fw"></i></button>
                    <button type="button" class="btn btn-default btn-xs" title="Xóa"
                        onclick="LoadDeleteItem('slider', {$data.id}, 'ajax_delete');"><i
            ...
            {else}
            <tr>
                <td colspan="10">Không tìm thấy mục nào</td>
            </tr>
            {/if}
        </tbody>
    </table>
</div>
<input type="hidden" id="cateId" value="{$smarty.get.id}">
...
<script>
    ...
    function LoadFormImage(id) {
        $.post("?mod=slider&site=image", {
            ...
            $("#FormImage select[name=type]").html(data.select_type);
    ...
    function SaveImage() {
        ...
        var type = $("#FormImage select[name=type]").val();
    ...
    function filter() {
        ...
        window.location.href = "?mod=slider&site=image" + "&type=" + type + "&id=" + category;
    }
