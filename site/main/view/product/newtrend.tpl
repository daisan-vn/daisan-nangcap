...
<header id="mheader" class="oem d-block d-sm-none">
	<div class="mtophead">
		<div class="d-flex justify-content-between bd-highlight p-4">
			...
			<div class="bd-highlight text-lg">Top Sản Phẩm Bán Chạy</div>
...
<div class="main-content-wrap">
	<div class="new-arrivals d-none d-sm-block">
		<div class="video-container">
			...
			{if $data.position eq 1}
			<img data-src="{$data.image}" class="img-fluid" loading="lazy">
			{/if}
			{/foreach}
		</div>
	</div>
							...
							<div class="card border-0 rounded-8">
								<div class="product-list-recommend">
									...
									<div class="list-item-img">
										<a href="{$v.url}"><img data-src="{$v.avatar}" alt="{$v.name}" class="img-fluid" loading="lazy"></a>
									</div>
...
$('.tab-item').click(function () {
	...
	var data = {};
	data.id = a_h[1];
	$('#Loaddb-' + a_h[1]).load('?mod=product&site=load_more_newtrend', data);
	a_tab.id = a_h[1];
	$(window).scrollTop(0);
	a_tab.page = 1;
});
...
$(window).scroll(function () {
	...
	if ($(window).scrollTop() >= h) {
		...
		if (a_tab.page < 11) {
			$.post('?mod=product&site=load_more_newtrend', a_tab, function (data) {
